...
- [x] **BUG-03**: `POST /api/runs` body is parsed via a Pydantic `RunConfig` model (not bare `dict`); endpoint validates input and appears correctly in `/docs`
- [x] **BUG-04**: `run_pipeline()` accepts `slot3_base_url` and `slot4_base_url` parameters; `server.py` reads `config.base_urls` and forwards them; `AsyncOpenAI` clients for Slots 3 & 4 use the dashboard-supplied base URL, not hardcoded env vars
