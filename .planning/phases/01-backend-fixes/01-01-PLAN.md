---
phase: 01-backend-fixes
plan: 01
type: execute
wave: 1
depends_on: []
files_modified:
  - vc_agents/web/server.py
  - vc_agents/pipeline/run.py
autonomous: true
requirements:
  - BUG-01
  - BUG-02

must_haves:
  truths:
    - "server.py imports _load_jsonl so any checkpoint resume path in the HTTP layer resolves without NameError"
    - "checkpoint resume after Stage 2 loads final_plans keyed by idea_id so Stage 3 can find the plan without KeyError"
    - "Stage 3's run_stage3 looks up final_plans by iterating values and matching on founder_provider, not by direct key access on founder.name"
  artifacts:
    - path: "vc_agents/web/server.py"
      provides: "_load_jsonl import from vc_agents.pipeline.run"
      contains: "from vc_agents.pipeline.run import _load_jsonl"
    - path: "vc_agents/pipeline/run.py"
      provides: "checkpoint resume keys final_plans by idea_id, Stage 3 looks up by founder_provider match"
      contains: 'p["idea_id"]: p for p in plans_list'
  key_links:
    - from: "vc_agents/web/server.py"
      to: "vc_agents/pipeline/run._load_jsonl"
      via: "import statement at top of file"
      pattern: "from vc_agents.pipeline.run import _load_jsonl"
    - from: "vc_agents/pipeline/run.run_pipeline (checkpoint path)"
      to: "run_stage3 final_plans lookup"
      via: "idea_id key in final_plans, Stage 3 matches on founder_provider"
      pattern: 'p\["idea_id"\]: p'
---

<objective>
Fix the two crash-on-resume bugs: import the missing `_load_jsonl` symbol in server.py (BUG-01) and rekey the checkpoint-loaded `final_plans` dict in run.py from `founder_provider` to `idea_id`, updating Stage 3's lookup to match (BUG-02).

Purpose: These are latent crashes that surface only on checkpoint resume paths. After this plan, a user can kill the server mid-pipeline and resume without NameError or KeyError.
Output: Two modified files with targeted one- to four-line fixes each.
</objective>

<execution_context>
@C:/Users/Vince/.claude/get-shit-done/workflows/execute-plan.md
@C:/Users/Vince/.claude/get-shit-done/templates/summary.md
</execution_context>

<context>
@.planning/PROJECT.md
@.planning/ROADMAP.md
@.planning/STATE.md
</context>

<interfaces>
<!-- Key code sections the executor must understand before editing. -->

From vc_agents/web/server.py (current imports, lines 9-27):
```python
from __future__ import annotations

import asyncio
import json
import os
import threading
import time
import uuid
from pathlib import Path
from typing import Any

from dotenv import load_dotenv
from fastapi import FastAPI, WebSocket, WebSocketDisconnect
from fastapi.responses import HTMLResponse, JSONResponse

from vc_agents.logging_config import setup_logging
from vc_agents.pipeline.cost_estimator import estimate_cost, load_catalog
from vc_agents.pipeline.events import EventCallback, PipelineEvent
from vc_agents.pipeline.run import run_pipeline
```

From vc_agents/pipeline/run.py (checkpoint resume section, lines 816-837):
```python
        # Stage 1: Ideate and Select
        if checkpoint and checkpoint.get("stage1_complete"):
            logger.info("Resuming: loading Stage 1 selections from checkpoint")
            selections_list = _load_jsonl(run_dir / "stage1_selections.jsonl")
            selections = {s["founder_provider"]: s for s in selections_list}
        else:
            selections = run_stage1(...)
            _save_checkpoint(run_dir, {"stage1_complete": True})

        # Stage 2: Build and Iterate
        if checkpoint and checkpoint.get("stage2_complete"):
            logger.info("Resuming: loading Stage 2 plans from checkpoint")
            plans_list = _load_jsonl(run_dir / "stage2_final_plans.jsonl")
            final_plans = {p["founder_provider"]: p for p in plans_list}
        else:
            final_plans = run_stage2(...)
            _save_checkpoint(run_dir, {"stage1_complete": True, "stage2_complete": True})
```

From vc_agents/pipeline/run.py (Stage 3 lookup, line 664):
```python
    for founder in founders_list:
        plan = final_plans[founder.name]   # KeyError if keyed by founder_provider not idea_id
```

From vc_agents/pipeline/run.py (_load_jsonl definition, lines 270-278):
```python
def _load_jsonl(path: Path) -> list[dict[str, Any]]:
    """Load records from a JSONL file."""
    records: list[dict[str, Any]] = []
    with path.open(encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line:
                records.append(json.loads(line))
    return records
```
</interfaces>

<tasks>

<task type="auto">
  <name>Task 1: Add _load_jsonl import to server.py (BUG-01)</name>
  <files>vc_agents/web/server.py</files>
  <action>
    In `vc_agents/web/server.py`, extend the existing `from vc_agents.pipeline.run import run_pipeline` line (line 27) to also import `_load_jsonl`. Change it to:

    ```python
    from vc_agents.pipeline.run import _load_jsonl, run_pipeline
    ```

    No other changes to server.py in this task. The spec's Option A (minimal) is correct — do NOT create a shared utils module (that is Option B and is out of scope for this fix).

    Keep import alphabetical within the `from vc_agents.pipeline.run import` statement: `_load_jsonl` comes before `run_pipeline`.
  </action>
  <verify>
    <automated>cd C:/Users/Vince/Desktop/VC_AI_Incubator-main && python -c "from vc_agents.web.server import app; print('import OK')"</automated>
  </verify>
  <done>Running `python -c "from vc_agents.web.server import app"` exits 0 with no NameError or ImportError. The import line contains both `_load_jsonl` and `run_pipeline`.</done>
</task>

<task type="auto">
  <name>Task 2: Rekey checkpoint final_plans by idea_id and fix Stage 3 lookup (BUG-02)</name>
  <files>vc_agents/pipeline/run.py</files>
  <action>
    In `vc_agents/pipeline/run.py`, make exactly two changes:

    **Change 1 — Stage 2 final_plans checkpoint resume (around line 831):**

    The STARTUP_PLAN_SCHEMA has `idea_id` at the top level, so `p["idea_id"]` is valid.
    Change the dict comprehension from:
    ```python
    final_plans = {p["founder_provider"]: p for p in plans_list}
    ```
    to:
    ```python
    final_plans = {p["idea_id"]: p for p in plans_list}
    ```

    Do NOT touch the Stage 1 selections loading line (`selections = {s["founder_provider"]: s for s in selections_list}`).
    SELECTION_SCHEMA has no top-level `idea_id` field — that line must remain keyed by `founder_provider`.

    **Change 2 — Stage 3 final_plans lookup (around line 664 in run_stage3):**

    The checkpoint resume path now keys `final_plans` by `idea_id`. The non-resume path (`run_stage2` return value) is keyed by `founder.name`. Both paths must share a consistent lookup strategy. Update Stage 3's lookup from:
    ```python
    plan = final_plans[founder.name]
    ```
    to:
    ```python
    plan = next((p for p in final_plans.values() if p["founder_provider"] == founder.name), None)
    if plan is None:
        logger.warning("No plan found for founder %s, skipping", founder.name)
        continue
    idea_id = plan["idea_id"]
    ```

    Place the `next(...)` call immediately before any use of `plan` in the loop body. If `idea_id = plan["idea_id"]` already exists on the next line, remove the duplicate — only one assignment is needed.

    This lookup works for BOTH the checkpoint resume path (keyed by idea_id) and the non-resume path (keyed by founder.name) because it searches values, not keys. It also adds a safe skip instead of a hard crash.

    Do NOT touch Stage 2 (run_stage2) — it builds final_plans correctly using founder.name as the key for the non-resume path, and that is fine since Stage 3 now iterates values.
  </action>
  <verify>
    <automated>cd C:/Users/Vince/Desktop/VC_AI_Incubator-main && python -c "from vc_agents.pipeline.run import run_pipeline; print('import OK')"</automated>
  </verify>
  <done>
    The Stage 2 checkpoint block uses `p["idea_id"]` as the key. Stage 3 uses `next((p for p in final_plans.values() if p["founder_provider"] == founder.name), None)` with a None guard. The module imports without error. The Stage 1 selections line is unchanged (`founder_provider` key).
  </done>
</task>

</tasks>

<verification>
After both tasks:
- `python -c "from vc_agents.web.server import app"` exits 0
- `python -c "from vc_agents.pipeline.run import run_pipeline"` exits 0
- `grep "_load_jsonl" vc_agents/web/server.py` shows the import line
- `grep "founder_provider.*plans_list" vc_agents/pipeline/run.py` returns no matches (the final_plans comprehension no longer uses founder_provider)
- `grep "selections.*founder_provider.*selections_list" vc_agents/pipeline/run.py` still returns the unchanged selections line
- Stage 3 loop body contains `next(` with `founder_provider` match, not a direct key lookup
</verification>

<success_criteria>
1. `server.py` imports `_load_jsonl` alongside `run_pipeline` — NameError on checkpoint resume path is eliminated
2. `final_plans` checkpoint dict in `run_pipeline` is keyed by `idea_id` — the mismatch with Stage 3 is resolved
3. Stage 3 looks up plans by iterating `final_plans.values()` and matching `founder_provider == founder.name` — works for both resume and non-resume paths, with a safe skip on None
4. Stage 1 selections loading line is untouched — still keyed by `founder_provider` as SELECTION_SCHEMA requires
</success_criteria>

<output>
After completion, create `.planning/phases/01-backend-fixes/01-01-SUMMARY.md` using the summary template.
</output>
