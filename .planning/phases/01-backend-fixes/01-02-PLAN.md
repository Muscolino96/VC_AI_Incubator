...
<task type="auto">
  ...
  <action>
    ...
    **Step 5 — Update _run_in_thread call:**
    ...
    Do NOT change `_run_in_thread` itself in this task — it still operates on a dict and forwards to `run_pipeline`. That forwarding is extended in Task 2.
...
<task type="auto">
  ...
  <action>
    NOTE: Task 1 prohibited touching `_run_in_thread` during Task 1 scope only. Task 2 intentionally updates it to add slot URL extraction — apply the full replacement shown below.
